...
def form_transaction_base(passoffile):
    ...
    passdb = passoffile[:passoffile.rfind('/')] +'/TransactionsBD.sqlite' #S.rfind(str, [start],[end])
    ...
    for row in range(2, df.shape[0]):
        ...
        # ff.form_name(data)
        # ff.form_gender(data)
        cursor.execute("INSERT OR REPLACE INTO TransactionsBD VALUES (?, ?, ?, ?, ?, ?, ?);", (data[0], data[1], data[2], data[3], data[4], data[5], data[6]))
...
# form_transaction_base(passoffile)
